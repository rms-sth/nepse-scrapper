...
class ShareSansarScrapper:
    ...
    def parse_columns(self, soup: BeautifulSoup) -> list:
        t_heads = soup.find_all("thead") # ['<thead>...</thead>',...]
        columns = [] # ['sn', 'symbold',...]
        for t_head in t_heads:
            for th in t_head.find_all("th"):
                th = th.text.replace("%", "percent")
                column = slugify(th) # Diff % => diff-percent
                column = column.replace("-", "_") # diff-percent => diff_percent 
                # 120_days => _120_days
    ...
    def parse_data(self, soup: BeautifulSoup):
        ...
        today_prices = self.parse_rows(soup, columns) # ['sn', 'symbold',...]
